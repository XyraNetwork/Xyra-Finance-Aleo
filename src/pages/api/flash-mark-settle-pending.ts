...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!body?.session_id?.trim() || !body?.flash_settle_tx_id?.trim()) {
    return res.status(400).json({ error: 'session_id and flash_settle_tx_id are required' });
  }

  try {
    const backendRes = await fetch(`${backendUrl}/flash/mark-settle-pending`, {
      method: 'POST',
      headers: {
    ...
    console.error('[api/flash-mark-settle-pending]', e);
    return res.status(500).json({ error: 'Failed to mark flash settle pending' });
  }
}
